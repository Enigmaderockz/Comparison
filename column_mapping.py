...
def compare_csv_files(
    ...
    with open(file1, "r") as f1, open(file2, "r") as f2, open(outfile, "w") as outfile:
        ...
        sorted_rows1_set = set(tuple(sorted_row.items()) for sorted_row in sorted_rows1)
        sorted_rows2_set = set(tuple(sorted_row.items()) for sorted_row in sorted_rows2)

        num_diff_records = int((len(sorted_rows1_set.symmetric_difference(sorted_rows2_set)))/2)
        num_records_file1_not_in_file2 = len(sorted_rows1_set.difference(sorted_rows2_set))
        num_records_file2_not_in_file1 = len(sorted_rows2_set.difference(sorted_rows1_set))
