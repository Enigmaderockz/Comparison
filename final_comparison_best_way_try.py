...
def compare_csv_files(
    ...
    with open(file1, "r") as f1, open(file2, "r") as f2, open(outfile, "w") as outfile:
        ...
        sorted_rows1 = process_batches(rows1, sort_keys, 5000)
        sorted_rows2 = process_batches(rows2, sort_keys, 5000)
        ...
        if num_diff_records == 0:
            ...
            with concurrent.futures.ThreadPoolExecutor() as executor:
                futures = [executor.submit(parallel_compare_rows, row1, row2) for row1, row2 in zip(sorted_rows1, sorted_rows2)]
